...
export const updateListing = async (req, res) => {
  try {
    ...
    if (image3) updatedData.image3 = image3;
...
export const deleteListing = async (req, res) => {
  try {
    ...
    // if (listing.image1) await cloudinary.uploader.destroy(getPublicId(listing.image1));
    // if (listing.image2) await cloudinary.uploader.destroy(getPublicId(listing.image2));
    // if (listing.image3) await cloudinary.uploader.destroy(getPublicId(listing.image3));
...
// Optional: extract public_id from Cloudinary URL if needed
const getPublicId = (url) => {
  const parts = url.split('/');
  const fileName = parts[parts.length - 1]; // example: abc123.png
  return fileName.split('.')[0];           // returns: abc123
};
